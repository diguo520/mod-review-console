...
// 自建部署(Cloudflare Pages): 默认走同源 /api/pb, 由 functions/api/pb/[[path]].ts 代理到
// 真正的 PocketBase; 想直连别的地址(本地调试)就设 VITE_PB_URL(如 http://127.0.0.1:8090)。
// 这里只放地址, 不放任何令牌。
const PB_URL_OVERRIDE = String(import.meta.env.VITE_PB_URL || "")
  .trim()
  .replace(/\/+$/, "")

// 浏览器侧用 VibeX 路径推 __pb 代理前缀; 本地直跑 dev (无 app-preview 前缀)时 fallback 到 /api/pb
export function getPocketBaseUrl(): string {
  if (PB_URL_OVERRIDE) return PB_URL_OVERRIDE
  const prefix = matchVibexPrefix()
  return prefix ? `${prefix}/__pb` : "/api/pb"
}
